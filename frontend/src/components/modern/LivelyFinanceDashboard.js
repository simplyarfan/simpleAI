  ...
  return (
    <div className="min-h-screen bg-black relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        <motion.div 
          className="absolute w-96 h-96 bg-emerald-500/5 rounded-full blur-3xl"
          animate={{
        ...
        />
        <motion.div 
          className="absolute w-64 h-64 bg-teal-500/3 rounded-full blur-2xl"
          animate={{
              ...
              >
                <div className="relative">
                  <div className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center shadow-lg border border-white/20">
                    <Sparkles className="w-5 h-5 bg-gradient-to-r from-emerald-400 to-teal-400 bg-clip-text text-transparent" style={{WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundImage: 'linear-gradient(to right, rgb(52 211 153), rgb(45 212 191))'}} />
                  </div>
                  <motion.div 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {financeAgents.map((agent, index) => (
                ...
                >
                  ...
                  <div className={`relative bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8 hover:border-white/20 transition-all duration-300 ${agent.available ? 'group-hover:transform group-hover:scale-[1.02]' : 'opacity-75'}`}>
                    <div className="flex items-start justify-between mb-6">
                      <div className="flex items-center space-x-4">
                        <motion.div 
                          className={`w-16 h-16 bg-white/10 border border-white/20 rounded-2xl flex items-center justify-center shadow-lg ${!agent.available ? 'grayscale' : ''}`}
