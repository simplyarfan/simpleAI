...
Provide detailed analysis as JSON:
{
  ...
  "candidate_summary": "A compelling 1-2 sentence summary of why this candidate would be valuable for this role, highlighting their key strengths and potential impact."
}`;
    ...
    // Fallback matching with basic logic
    const skillsMatched = cvData.skills.filter(skill => 
      jdData.required_skills.some(reqSkill => 
        skill.toLowerCase().includes(reqSkill.toLowerCase()) ||
        reqSkill.toLowerCase().includes(skill.toLowerCase())
      )
    );
    
    const skillsMissing = jdData.required_skills.filter(reqSkill => 
      !cvData.skills.some(skill => 
        skill.toLowerCase().includes(reqSkill.toLowerCase()) ||
        reqSkill.toLowerCase().includes(skill.toLowerCase())
      )
    );
    
    const matchPercentage = Math.round((skillsMatched.length / jdData.required_skills.length) * 100);
    ...
    return {
      ...
      recommendations: ['Consider for interview if other qualifications are strong'],
      fit_level: matchPercentage >= 70 ? 'High' : matchPercentage >= 50 ? 'Medium' : 'Low',
      recommendation: matchPercentage >= 70 ? 'Recommended' : 'Consider',
      candidate_summary: `${cvData.personal.name} brings ${cvData.experience.length > 0 ? cvData.experience[0].position.toLowerCase() : 'relevant'} experience with strong technical skills in ${skillsMatched.slice(0, 3).join(', ')}, making them a solid candidate for this ${jdData.position_title.toLowerCase()} role.`
    };
  }
}
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (event.httpMethod === 'POST' && (path === '' || path === '/')) {
      ...
      // Add completed batch to batches list
      batches.push(batch);
