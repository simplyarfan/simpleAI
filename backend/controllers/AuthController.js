...
class AuthController {
  ...
  static async createUser(req, res) {
    try {
      ...
      res.status(201).json({
        ...
        data: {
          user: user.toJSON(),
          tempPassword: send_invitation ? undefined : tempPassword
        }
      });
  ...
  static async updateUser(req, res) {
    try {
      ...
      // Update email and role separately if provided (requires direct database update)
      if (email || role || is_active !== undefined) {
        const updates = [];
        const params = [];
        let paramIndex = 1;
