...
class SupportController {
  ...
  static async createTicket(req, res) {
    try {
      ...
      `, [
        ...
        JSON.stringify({ ticket_id: result.id, subject, priority, category }),
        req.ip,
        req.get('User-Agent')
      ]);
  ...
  static async getTicket(req, res) {
    try {
      ...
      const comments = await database.all(`
        ...
        WHERE tc.ticket_id = ?
        AND (tc.is_internal = 0 OR ? = 1)
        ORDER BY tc.created_at ASC
      `, [ticket_id, req.user.isAdmin ? 1 : 0]);
  ...
  static async addComment(req, res) {
    try {
      ...
      const result = await database.run(`
        ...
        VALUES (?, ?, ?, ?)
      `, [ticket_id, req.user.id, comment, isInternalComment ? 1 : 0]);
      ...
      const assigneeStats = await database.all(`
        SELECT 
          ...
          AVG(CASE 
            ...
            THEN (julianday(st.resolved_at) - julianday(st.created_at)) * 24 
          END) as avg_resolution_time_hours
        FROM users u
  ...
  static async deleteTicket(req, res) {
    try {
      ...
      await database.run('DELETE FROM support_tickets WHERE id = ?', [ticket_id]);
